...
async function onlclick(d) {   //left click  for  red player
    ...
    fill_Proportion()
    filTxt()
    if (Max_Choice_Num - cur_red_choice_num==0) {
        next = 'blue'
    }
    this.run(tree)
}
...
async function Solve() {   //solve button for blue player
    ...
    for(;i<length;i++){
        ...
        let source = parseInt(Recording[i].split(":")[2])
        console.log("source:"+source)
        let dest  = calculate_Position(source,(beginLabel+1)/(beginLabel+1-length))
        console.log("dest:"+dest)
        single.handle(dest, source)
    }
    treemap.run(tree)
}

//this is George's function, undecided
function calculate_Position(source,scale){
    ...
    for(let i = beginLabel;i<=endLabel;i++){
        ...
        if((i-beginLabel+1-redNum)*scale>=(source-beginLabel)){
            destination = i
            break
        }
    }
